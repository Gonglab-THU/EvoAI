...
if [ ${metric} == 'ddGdTm' ]; then
    ...
    if [ ${version} == 'Seq' ]; then
        ...
        python ${GEOSTAB_DIR}/model_ddG_Seq/predict.py --ensemble_feature ${mut_folder}/ensemble.pt --saved_folder ${mut_folder}
        python ${GEOSTAB_DIR}/model_dTm_Seq/predict.py --ensemble_feature ${mut_folder}/ensemble.pt --saved_folder ${mut_folder}
    fi
fi
