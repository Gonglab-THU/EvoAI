...
from relative_coordinate import *
...
class PretrainGeometricAttention(torch.nn.Module):
    ...
    def __init__(self, node_dim, n_head, pair_dim, qk_point_dim = 8, v_point_dim = 8):
        ...
        self.out_transform = torch.nn.Sequential(
            torch.nn.Linear((n_head * pair_dim) + (n_head * v_point_dim * (3 + 3 + 1)) + (n_head * 7), node_dim * 2),
...
class PretrainEncoder(torch.nn.Module):
    ...
    def __init__(self, node_dim, n_head, pair_dim, num_layer):
        ...
        self.fixed_embedding_transform = torch.nn.Sequential(
            torch.nn.Linear(node_dim + 7, node_dim),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(node_dim, node_dim)
        )
...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, n_layer):
        ...
        if n_layer == 1:
            ...
            self.read_out = torch.nn.Sequential(
